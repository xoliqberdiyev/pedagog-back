...
from apps.pedagog.serializers.topic import MobileTopicDetailSerializer
...
class SearchApiView(views.APIView):
    ...
    def get(self, request):
        ...
        topics = Topic.objects.filter(name__istartswith=text)[:limit]
        categories = ElectronResourceCategory.objects.filter(name__istartswith=text)[:limit]
        subcategories = ElectronResourceSubCategory.objects.filter(name__istartswith=text)[:limit]
        resources = ElectronResource.objects.filter(name__istartswith=text)[:limit]

        data = {
            "topics": MobileTopicDetailSerializer(topics, many=True).data,
